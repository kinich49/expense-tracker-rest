...
public class PaymentMethodRequestCondition implements Condition<TransactionRequestParameter> {
    ...
    public Optional<ErrorWrapper> assertCondition(TransactionRequestParameter param) {
        ...
        if (paymentMethodRequest == null) {
            ...
            return Optional.of(new ErrorWrapper(TransactionServiceErrorCodes.PAYMENT_METHOD_REQUEST_IS_NULL_OR_EMPTY,
                    errorMessage));
        }

        if (NumberUtils.isEitherNullZeroOrNegative(paymentMethodRequest.getId()) &&
                StringUtils.isNullOrEmptyOrBlank(paymentMethodRequest.getName())) {
            var errorMessage = "Payment method must have an id and/or name. ";
            return Optional.of(new ErrorWrapper(TransactionServiceErrorCodes.PAYMENT_METHOD_REQUEST_HAS_NO_NAME_OR_ID,
                    errorMessage));
        }
